...
const {ENV, base, shop, getOption} = require('./env');
...
const shopwareBase = () => {
  ...
  let yoBase = findUp.sync('.yo-rc.json');
  if (yoBase) {
    yoBase = path.dirname(yoBase);
  }

  const cmd = spawn.sync('git', ['rev-parse', '--show-toplevel']);
  const basePath = base || (cmd.status === 0 && cmd.stdout.toString().trim()) || yoBase || process.cwd();
...
if (!fs.existsSync(template)) {
  ...
  fs.ensureSymlinkSync(path.join(__dirname, '../..'), template);
}
