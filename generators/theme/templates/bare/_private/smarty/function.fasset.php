 ...
 */
function smarty_function_fasset($params, $template)
{
    $file = $params['file'];
    ...
    $request = $front->Request();
      ...
      //Shopware->Container()->getParameter('kernel.environment') ne 'node'
    $useManifest = Shopware()->Container()->getParameter('kernel.environment') != 'node';

    // check if we got an URI or a local link
    if (!empty($file) && strpos($file, '/') !== 0 && strpos($file, '://') === false) {

        // first try convinient
        if ($useManifest && array_key_exists('frontend/_resources/'.$file, $manifest)) {
            $file = $manifest['frontend/_resources/'.$file];
        } elseif ($useManifest && array_key_exists($file, $manifest)) {
            $file = $manifest[$file];
        }
